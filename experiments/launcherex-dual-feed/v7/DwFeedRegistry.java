...
/** V7 diagnostic source corresponding to the classes2-only device candidate. */
final class DwFeedRegistry {
    ...
    static void create(String id, String page, int width, int height, int density,
        ...
        if (id == null || output == null || !output.isValid() || width <= 0 || height <= 0 ||
                !(DwFeedProtocol.PAGE_HOME.equals(page) || DwFeedProtocol.PAGE_RECENT.equals(page))) {
            sendError(reply, id, "bad_request");
            return;
        }
        Session s = new Session();
        s.id = id; s.page = page; s.width = width; s.height = height;
        s.density = Math.max(120, density); s.reply = reply; s.output = output;
    ...
    private static String launchViaShizuku(Session s) {
        ...
        try {
            ...
            String cmd = "/system/bin/am start -W --user current --display " + displayId +
                    " -f 0x18010000 -n com.mekromn.dwfilemanager/dw.filemanager.feed.DwFeedActivity" +
                    " --es session " + s.id + " 2>&1";
    ...
    static void activityReady(ExplorerActivity activity, String id) {
        ...
        if (s == null) { activity.finish(); return; }
        s.activity = activity;
        try {
    ...
    private static void markReady(Session s) {
        ...
        try {
            Bundle b = new Bundle(); b.putString(DwFeedProtocol.K_SESSION, s.id);
            Message m = Message.obtain(null, DwFeedProtocol.MSG_READY); m.setData(b);
            s.reply.send(m); s.ready = true;
        } catch (Throwable t) { fail(s, "ready_failed:" + describe(t)); }
    }

    static void input(String id, MotionEvent event) {
        Session s = SESSIONS.get(id);
        if (s == null || s.activity == null || event == null) {
            if (event != null) event.recycle();
            return;
        }
        MotionEvent copy = MotionEvent.obtain(event); event.recycle();
        MAIN.post(() -> {
            try { if (SESSIONS.get(id) == s && s.activity != null) s.activity.dispatchTouchEvent(copy); }
    ...
    private static void cleanup(Session s) {
        try { if (s.launchProcess != null) s.launchProcess.destroy(); } catch (Throwable ignored) {}
    ...
    private static String describe(Throwable t) {
        ...
        String type = t.getClass().getSimpleName(), msg = t.getMessage();
    ...
    private static void sendError(Messenger reply, String id, String why) {
        ...
        try {
            Bundle b = new Bundle(); b.putString(DwFeedProtocol.K_SESSION, id); b.putString(DwFeedProtocol.K_ERROR, why);
            Message m = Message.obtain(null, DwFeedProtocol.MSG_ERROR); m.setData(b); reply.send(m);
