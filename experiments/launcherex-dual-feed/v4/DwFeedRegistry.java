...
/** V4 Android 16 host. Shizuku is used only to launch DW's own activity on the already-created display. */
final class DwFeedRegistry {
    ...
    private static String launchViaShizuku(Session s){
        ...
        try{
            ...
            int code=p.waitFor();
            return code==0?null:"shizuku_exit_"+code;
    ...
    private static String describe(Throwable t){if(t==null)return "Unknown";String type=t.getClass().getSimpleName(),msg=t.getMessage();if(msg==null||msg.length()==0)return type;msg=msg.replace('\n',' ').replace('\r',' ').replace('\t',' ');while(msg.contains("  "))msg=msg.replace("  "," ");if(msg.length()>180)msg=msg.substring(0,180);return type+":"+msg;}
